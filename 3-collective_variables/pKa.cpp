...
class pKa : public pKaBase{
  SwitchingFunction switchingFunction;

public:
  explicit pKa(const ActionOptions&);
// active methods:
...
PLUMED_REGISTER_ACTION(pKa,"PKA")

void pKa::registerKeywords( Keywords& keys ){
  pKaBase::registerKeywords(keys);
...
pKa::pKa(const ActionOptions&ao):
Action(ao),
pKaBase(ao) {}
//{
//
...
double pKa::sw_func(double arg, double sw_cutoff, double sw_shift, int p, int q){
 double sw;
...
double pKa::dsw_func(double arg, double sw_cutoff, double sw_shift, int p, int q){
 double d_sw;
 if(abs(arg-sw_shift)/sw_cutoff>(1.-100.0*epsilon) && abs(arg-sw_shift)/sw_cutoff<(1+100.0*epsilon)){
...
//double pKa::pairing(double arg,double&dfunc, double sw_cutoff, double sw_shift, int p, int q){
// sw = (1-pow(((arg-sw_shift)/sw_cutoff),p))/(1-pow(((arg-sw_shift)/sw_cutoff),q));
